...
## Phase 8 — 예매 Pan/Zoom 좌석 선택 UI 🔲

> **목적**: Ticketmaster/티케톤 스타일의 pan/zoom 기반 좌석 선택 UX 구현
> **선행 조건**: Phase 7 완료 ✅
> **사용 에셋**: `public/venue/KSPODOME.svg`, `public/venue/KSPODOME-grape.svg`, `public/icons/seat-*.svg`

### 아키텍처

```
seats-section  ─┐
                ├→ VenueZoomCanvas (통합 pan/zoom 캔버스)
seats-individual┘
```

- 배경: `KSPODOME-grape.svg` (6181×4820)
- SVG overlay: 구역 rect (seats-section) 또는 좌석 dot (seats-individual)
- 좌표 변환: `GRAPE_SCALE = 6181 / 895 ≈ 6.907` (이미 `shared/lib/venue.ts`에 정의됨)

### 8-1. VenueZoomCanvas 컴포넌트

- [ ] `src/features/booking/ui/VenueZoomCanvas.tsx` 신규 생성
  - [ ] pan/zoom 상태 관리: `{ x, y, scale }` + `useRef` 컨테이너
  - [ ] 마우스 휠: 커서 방향으로 줌 (0.05x ~ 6x)
  - [ ] 마우스 드래그: 패닝
  - [ ] 배경: `<img src="/venue/KSPODOME-grape.svg">` (6181×4820)
  - [ ] 오버레이: `<svg viewBox="0 0 6181 4820">` — 구역 rect 또는 좌석 dot

### 8-2. seats-section 모드

- [ ] `GRAPE_SECTION_BBOXES` 기반 구역별 반투명 rect 렌더
- [ ] 구역 hover: 구역명 + 잔여석 툴팁
- [ ] 구역 클릭: `transitionTo("seats-individual")` + 해당 구역으로 자동 줌인 (300ms animate)
- [ ] 전체 도돔 fit 초기 줌

### 8-3. seats-individual 모드

- [ ] 선택 구역 자동 줌인 (mount 시 animate)
  - `scale = min(containerW / gb.w, containerH / gb.h) * 0.85`
- [ ] 좌석 dot 렌더: `GRAPE_SECTION_BBOXES` 내 행×열 그리드
  - [ ] `<image href="/icons/seat-enabled.svg">` — 선택 가능
  - [ ] `<image href="/icons/seat-selected.svg">` — 내가 선택
  - [ ] `<image href="/icons/seat-disabled.svg">` — 판매완료/잠금
- [ ] 좌석 클릭: `toggleSeat` (최대 4석)
- [ ] 드래그로 인접 좌석 영역 탐색
- [ ] 3분 타이머 연동 (`useSeatTimer` 기존 그대로)
- [ ] `useSeatLockSimulation` 연동 (기존 그대로)

### 8-4. UnifiedSeatView 교체

- [ ] `UnifiedSeatView.tsx` — 기존 VenueMap + SeatOverlay 중앙 영역을 `VenueZoomCanvas`로 교체
- [ ] 줌 컨트롤 버튼 (선택): `+` / `−` / 리셋 (우측 하단)
- [ ] 기존 `VenueMap`, `SeatOverlay` — Minimap이 여전히 사용하므로 유지

### 8-5. 기타

- [ ] `features/booking/index.ts` — `VenueZoomCanvas` 배럴 추가
- [ ] `npm run build` 통과 확인
