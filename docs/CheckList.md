...
| 양도 등록             | `POST /transfer/listings`                      | ✅   | `TransferListingModal` UI + API 연동 완료    |
| 양도 구매             | `POST /transfer/listings/{listingId}/purchase` | ✅   | `TransferPurchaseSidebar` UI + API 연동 완료 |
...
| 양도 등록·구매 유저 플로우 검증       | 🟡 보통  | API는 통합됨. Toss 결제 리다이렉션, 주문 ID 저장, callback 후 invalidation 흐름을 현장에서 확인 필요                                                    |
