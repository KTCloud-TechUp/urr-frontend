...
| 분야         | 기술                       | 비고                      |
| ------------ | -------------------------- | ------------------------- |
| Framework    | Next.js 15 (App Router)    | 서버/클라이언트 통합      |
...
| ---------- | ------------------ | -------------- | ----------- | ----------- | -------- |
| 💎 Diamond | 팬 신뢰 점수 ≥ 85  | 선예매 1순위   | +₩1,000     | 5%          | **면제** |
| 🥇 Gold    | 팬 신뢰 점수 66–84 | 2순위 (+1h)    | +₩2,000     | 5%          | **면제** |
| 🥈 Silver  | 멤버십 가입 즉시   | 3순위 (+2days) | +₩3,000     | 10%         | 필수     |
| 🥉 Bronze  | 회원가입 즉시      | 일반예매만     | +₩4,000     | 10%         | 필수     |

- **Diamond/Gold**: VQA 스킵 → 대기열 직행 (Fast Track)
- **Silver/Bronze**: VQA 통과 후 대기열 진입 (Standard Path)
- **양도 가능**: 전 등급 (Silver/Bronze는 수수료 10%)
...
- MVP: 멜론 미연동 → Bronze 유지. 멜론 연동 시에만 상위 등급 가능.
...
- Silver/Bronze 전용 (Diamond/Gold 면제)
...
3. **약관 동의**:
   ...
   - [필수] 이용약관 / [필수] 개인정보처리방침
   - [선택] 마케팅 수신 동의
4. 가입 완료 → **Bronze 즉시 부여**

**Phase 2 — 개인화** 5. 아티스트 선택 (1명 이상 필수, 카테고리 탭 + 검색) 6. 멤버십 소개: 등급 혜택 비교표 + [가입 ₩30,000/년] or [나중에] 7. 멜론 연동 (선택): "Diamond/Gold 등급 확인" CTA — 강제 아님 8. 완료 → 개인화된 홈으로 전환
