...
2. [기술 스택](#기술-스택)
3. [아키텍처](#아키텍처)
4. [인증 전략 — JWT + httpOnly 쿠키](#인증-전략--jwt--httponly-쿠키)
5. [프로토타입 → Next.js 마이그레이션](#프로토타입--nextjs-마이그레이션)
6. [핵심 기능](#핵심-기능)
7. [화면 구성](#화면-구성)
8. [개발 방식 — AI 에이전트 협업](#개발-방식--ai-에이전트-협업)
9. [로컬 실행](#로컬-실행)
10. [트러블슈팅](#트러블슈팅)
11. [관련 문서](#관련-문서)
...
| 기존 문제                 | URR의 접근                              |
| ------------------------- | --------------------------------------- |
| 매크로·봇이 대기열 점령   | VQA 팬 인증 시스템으로 자동화 접근 차단 |
...
```
VQA 팬 인증 → 실시간 대기열 → 구역 선택 → 좌석 선택 → 결제 → 예매 확인
```

- **VQA 팬 인증**: 아티스트 관련 텍스트 퀴즈로 매크로 차단 (3문제, 2/3 통과 시 입장). / Diamond/Gold 등급은 면제.
...
| `/events/:id/booking` | 예매 플로우     | VQA 팬 인증 → 실시간 대기열 → 구역 선택 → 좌석 선택 → 결제 → 예매 확인 |
...
초기 미들웨어는 `refresh_token` 쿠키만 확인했는데, 백엔드가 이 쿠키를 내려주지 않는 환경(로컬 개발, 쿠키 도메인 불일치 등)에서는 차단이 동작하지 않았습니다. 또한 미들웨어가 통과되더라도 컴포넌트 마운트 시점에 메모리 토큰이 없으면 로그인 화면을 그대로 렌더링하는 문제도 있었습니다.

**해결 방법**

세 곳을 동시에 수정했습니다.

**1. `tokenStore` — 클라이언트 쿠키 자동 관리**

Access Token을 메모리에 저장할 때 일반 쿠키(`is_authenticated=1`)를 동시에 설정합니다. JS에서 읽을 수 있어 미들웨어가 활용할 수 있습니다.
...
**2. `middleware.ts` — 이중 쿠키 확인**

`refresh_token` (httpOnly) 또는 `is_authenticated` (일반) 중 하나라도 있으면 인증 상태로 판단합니다.
...
**3. `OnboardingWidget` — 마운트 시 세션 재확인**

미들웨어를 통과하더라도, 마운트 시점에 `reissueToken()`을 호출해 실제 세션 유효 여부를 확인합니다. 유효하면 홈으로 리다이렉트합니다.
